      ...
      <div className="text-center">
        <div className="m-auto mb-3 w-full max-w-4xl flex justify-center items-center">
          {isLoading ? (
            ...
            <>
              <h2 className="ml-2">עיר: {city}</h2>
              <button
                className="bg-black hover:text-black hover:bg-white text-white font-bold py-2 px-4 shadow-md rounded transition-all duration-200 ease-in-out"
                onClick={() => getCity()}
              >
                דייק מיקום
              </button>
        ...
        <Login
          renderWhenUserLoggedIn={(user) => {
            return user ? (
              <UserContext.Provider value={{ city, email: "" }}>
                <FileUploader />
